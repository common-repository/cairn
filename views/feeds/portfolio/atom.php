...
<feed
 ...
 >
	...
	<entry>
		...
		<content type="<?php html_type_rss(); ?>" xml:base="<?php print $item['permalink']; ?>"><![CDATA[<?php print Cairn_Portfolio_Post::body( $item, false ); ?>]]></content>

<?php foreach ( $item['videos'] as $enclosure ) { 
	if ( $enclosure['feed'] ) {
		print '<enclosure url="' . trim(htmlspecialchars($enclosure['url'])) . '" length="' . trim($enclosure['meta']['filesize']) . '" type="'.trim($enclosure['type']).'" />'; // add type
	}
} ?>
	</entry>
	<?php } ?>
